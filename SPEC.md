...
-   **状態**:
    -   **季節**: L5層の平均温度と、その時間変化率から「春夏秋冬」が動的に決定される。
    -   **天気（感情）**: L5層の平均温度と平均ストレスから「喜・楽・哀・怒」が動的に決定される。
    -   UI上では「季節・天気」の形式で表示（例: `冬・楽`）。
-   **粒子の挙動**:
...
システムのコアロジックは、層ごとに定義された以下の数式に基づいて計算されるべきです。
...
#### 4.4. 必須観測指標 (Academic Observation Metrics)
...
2.  **支配的感情名 (Dominant Emotion):** **光源を含む19粒子**の中で、現在**温度 $T$ または $\Gamma_n$** が最大値を持つ粒子の名称。感情システムの**現在の活性化傾向**を特定する。
...
- [ ] **多層構造の実装:**
  - [ ] L0〜L4層の粒子と物理演算を実装する。
  - [ ] 各層の物性（Gas, Liquid, Solid）に応じた挙動を実装する。
- [ ] **創発重力と中心核の実装:**
  - [ ] 情報密度 `ρ_n` から創発重力定数 `G_eff` を動的に計算する。
  - [ ] 中心核からの引力 `F_Core` を全粒子に適用する。
- [ ] **自己組織化ループの実現:**
  - [ ] 内部状態（`S_n`）が `π_n` や `ρ_n` を通じて物理法則（重力・慣性）を変化させるフィードバックループを実装する。
- [ ] **高度な視覚フィードバックの実装:**
