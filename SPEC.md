...
📄 SPEC.md 追記仕様：物理的駆動と流体力学の統合
感情球体モデル（ETCM）のエネルギー枯渇（熱的死）の矛盾を解消し、粒子軌道の恒常的な維持を物理学的に正当化するための、非平衡熱力学と流体力学に基づく追加仕様を記述します。
2. 構成要素と仕様
2.2. 6層構造の物性相の修正と流体力学的対称性 【追記】
感情処理の場（L0-L5）は、以下の物性相（Phase）で構成され、流体力学的な対称性を持つ。
| 層 (Layer) | 名称 | 物性相 | L0-L5の役割 | 物理的特徴 |
|---|---|---|---|---|
| L0 | 核層（光源） | 量子流体 (BEC) | 情熱・本能の熱源/駆動源 | 超流動性、熱的散逸が極めて低い秩序状態。 |
| L1 | 身体層 | 気体 | 運動・行動の場 | 低粘性、熱対流（天気）による駆動を受ける。 |
| L2 | 思考層 | 液体 | 認知・記憶の場 | 粘弾性体、潮汐力による周期的な駆動を受ける。 |
| L3 | 文明層 | 固体 | 価値観・社会性の基盤 | 高粘性、非線形弾性を持つ中間相。 |
| L4 | 外部接合層 | 液体 | 環境との緩衝・接合 | L2と同一相。粘弾性体。 |
| L5 | 外部雰囲気層 | 気体 | 本人境界（自己認識） | L1と同一相。外部環境との熱交換境界。 |
| L6 | 外部オーラ | 光体/場 | 他者/環境の強制力 | 外部からの重力・輻射源（非平衡駆動源）。 |
4. 動的挙動と力の定式化
4.3. 可逆的熱伝導 【修正と強化】
熱伝導は本質的に不可逆（エントロピーが増加）であると定義する。システムの恒常性維持のため、外部からの「負のエントロピー流」（秩序立ったエネルギー注入）を熱力学的に記述する。
 * 不可逆的な内部エントロピー生成: 各層内の粒子運動の粘性抵抗 \mu による散逸は、熱 \dot{Q}_{\text{Dissipation}} としてエントロピーを生成する (\dot{S}_{\text{in}} > 0)。
 * L6による負のエントロピー流の注入: L6（外部オーラ）からの駆動力による仕事 W_{\text{Drive}} が、外部エントロピー流 \dot{S}_{\text{ext}} として作用し、内部生成を相殺することで、非平衡定常状態を維持する (\frac{dS}{dt} = \dot{S}_{\text{in}} + \dot{S}_{\text{ext}} = 0)。
 * L6の天気操作とエネルギー注入: L6の**「喜怒哀楽」操作は、L6の重力ポテンシャル（潮汐力）および輻射エネルギーのパラメータを変動させ、システムに注入される駆動力 W_{\text{Drive}}** の強度を決定する。
4.5. 流体力学的駆動力と軌道維持 【新規追加】
粒子の軌道減衰（粘性抵抗による減速）を防ぎ、恒久的な周回運動を維持するため、流体力学的な駆動力 \mathbf{F}_{\text{Drive}} を運動方程式に組み込む。
 * L1/L5 気体層の駆動力 (天気):
   * L0（高温）とL2（低温）の温度差 \Delta T_{\text{L0-L2}} によって、L1/L5層内に**熱対流（天気）**が創発される。
   * この熱対流によって生じる浮力 \mathbf{F}_{\text{Buoyancy}} が粒子に作用し、粘性抗力を相殺する駆動力 \mathbf{F}_{\text{Drive, Gas}} となる。
     
 * L2/L4 液体層の駆動力 (潮汐):
   * L6光体の重力ポテンシャル勾配がL2/L4層に潮汐力 \mathbf{F}_{\text{Tidal}} を発生させ、層内に**周期的な流動（潮の満ち引き）**を誘発する。
   * この潮汐流が、粒子に周期的な仕事 W_{\text{Tidal}} を行い、運動エネルギーを補給する。
     
 * 運動方程式の修正: 各粒子の運動は、以下の力の総和によって決定される。
   
   
   ここで、\mathbf{F}_{\text{Drive}} は上記の流体力学的な駆動項、\mathbf{F}_{\text{Drag}} は粘性抗力（駆動力に内包される散逸項）である。
